...
#### DESeq T6 vs T5 ####
calf_plus1_T6_T5 <- transform_sample_counts(calf_phyloseq_no_diet_and_T9, function(x) x+1)
calf_sex_deseq_T6_T5 <- phyloseq_to_deseq2(calf_plus1_T6_T5, ~`host_age`)
calf_sex_deseq_T6_T5$host_age <- relevel(calf_sex_deseq_T6_T5$host_age, ref = "T5")
DESEQ_calf_T6_T5<- DESeq(calf_sex_deseq_T6_T5)

resultsNames(DESEQ_calf_T6_T5)
# [1] "Intercept"         "host_age_T1_vs_T5" "host_age_T2_vs_T5" "host_age_T3_vs_T5" "host_age_T4_vs_T5"
# [6] "host_age_T6_vs_T5" "host_age_T7_vs_T5" "host_age_T8_vs_T5"

res_calf_T6_T5 <- results(DESEQ_calf_T6_T5, name = "host_age_T6_vs_T5", tidy = TRUE)
View(res_calf_T6_T5)


# To get table of results
sigASVs_T6_T5 <- res_calf_T6_T5 %>% 
  filter(padj<0.01 & abs(log2FoldChange)>2) %>%
  dplyr::rename(ASV=row)
View(sigASVs_T6_T5)
#To get ASV
sigASVs_T6_T5_vec <- sigASVs_T6_T5 %>%
  pull(ASV)

# Prune phyloseq file
T6_T5_DESeq <- prune_taxa(sigASVs_T6_T5_vec,calf_phyloseq_no_diet_and_T9)
sigASVs_T6_T5_merge <- tax_table(T6_T5_DESeq) %>% as.data.frame() %>%
  rownames_to_column(var="ASV") %>%
  right_join(sigASVs_T6_T5) %>%
  arrange(log2FoldChange) %>%
  mutate(Genus = make.unique(Genus)) %>%
  mutate(Genus = factor(Genus, levels=unique(Genus))) %>%
  mutate(Score = -log10(padj) * abs(log2FoldChange))  %>%
  arrange(desc(Score)) %>%
  slice_head(n = 10)
View(sigASVs_T6_T5_merge)

#graph volcano plot
vol_plot_T6_T5_upd <- res_calf_T6_T5 %>%
  mutate(significant = padj<0.01 & abs(log2FoldChange)>2) %>%
  ggplot() +
  geom_point(aes(x=log2FoldChange, y=-log10(padj), col=significant),size = 2, alpha = 0.8) + 
  scale_color_manual(
  ...
  geom_label_repel(
    data = sigASVs_T6_T5_merge, # Use the filtered data for only the points you want to label
    aes(x = log2FoldChange, y = -log10(padj), label = Genus),
    size = 3.5,
    box.padding = 0.6,
    point.padding = 0.5
...
ggsave("calf_DEG/labelled_graph/vol_plot_T6_T5_upd.png", vol_plot_T6_T5_upd, width = 8, height = 6.5, dpi = 300, bg = "white")


#### DEseq T8 vs T5 ####
calf_plus1_T8_T5 <- transform_sample_counts(calf_phyloseq_no_diet_and_T9, function(x) x+1)
calf_sex_deseq_T8_T5 <- phyloseq_to_deseq2(calf_plus1_T8_T5, ~`host_age`)
calf_sex_deseq_T8_T5$host_age <- relevel(calf_sex_deseq_T8_T5$host_age, ref = "T5")
DESEQ_calf_T8_T5<- DESeq(calf_sex_deseq_T8_T5)

resultsNames(DESEQ_calf_T8_T5)
# [1] "Intercept"         "host_age_T1_vs_T5" "host_age_T2_vs_T5" "host_age_T3_vs_T5" "host_age_T4_vs_T5"
# [6] "host_age_T6_vs_T5" "host_age_T7_vs_T5" "host_age_T8_vs_T5"

res_calf_T8_T5 <- results(DESEQ_calf_T8_T5, name = "host_age_T8_vs_T5", tidy = TRUE)
View(res_calf_T8_T5)


# To get table of results
sigASVs_T8_T5 <- res_calf_T8_T5 %>% 
  filter(padj<0.01 & abs(log2FoldChange)>2) %>%
  dplyr::rename(ASV=row)
View(sigASVs_T8_T5)
#To get ASV
sigASVs_T8_T5_vec <- sigASVs_T8_T5 %>%
  pull(ASV)

# Prune phyloseq file
T8_T5_DESeq <- prune_taxa(sigASVs_T8_T5_vec,calf_phyloseq_no_diet_and_T9)
sigASVs_T8_T5_merge <- tax_table(T8_T5_DESeq) %>% as.data.frame() %>%
  rownames_to_column(var="ASV") %>%
  right_join(sigASVs_T8_T5) %>%
  arrange(log2FoldChange) %>%
  mutate(Genus = make.unique(Genus)) %>%
  mutate(Genus = factor(Genus, levels=unique(Genus))) %>%
  mutate(Score = -log10(padj) * abs(log2FoldChange))  %>%
...
#graph volcano plot
vol_plot_T8_T5_upd <- res_calf_T8_T5 %>%
  mutate(significant = padj<0.01 & abs(log2FoldChange)>2) %>%
  ggplot() +
  geom_point(aes(x=log2FoldChange, y=-log10(padj), col=significant),size = 2, alpha = 0.8) + 
  scale_color_manual(
  ...
  geom_label_repel(
    data = sigASVs_T8_T5_merge, # Use the filtered data for only the points you want to label
    aes(x = log2FoldChange, y = -log10(padj), label = Genus),
    size = 3.5,
    box.padding = 0.6,
    point.padding = 0.5
...
ggsave("calf_DEG/labelled_graph/vol_plot_T8_T5_upd.png", vol_plot_T8_T5_upd, width = 8, height = 6.5, dpi = 300, bg = "white")
...
#### DEseq T8 vs T6 ####
calf_plus1_T8_T6 <- transform_sample_counts(calf_phyloseq_no_diet_and_T9, function(x) x+1)
calf_sex_deseq_T8_T6 <- phyloseq_to_deseq2(calf_plus1_T8_T6, ~`host_age`)
calf_sex_deseq_T8_T6$host_age <- relevel(calf_sex_deseq_T8_T6$host_age, ref = "T6")
DESEQ_calf_T8_T6<- DESeq(calf_sex_deseq_T8_T6)

resultsNames(DESEQ_calf_T8_T6)
# [1] "Intercept"         "host_age_T1_vs_T6" "host_age_T2_vs_T6" "host_age_T3_vs_T6" "host_age_T4_vs_T6"
# [6] "host_age_T5_vs_T6" "host_age_T7_vs_T6" "host_age_T8_vs_T6"

res_calf_T8_T6 <- results(DESEQ_calf_T8_T6, name = "host_age_T8_vs_T6", tidy = TRUE)
View(res_calf_T8_T6)


# To get table of results
sigASVs_T8_T6 <- res_calf_T8_T6 %>% 
  filter(padj<0.01 & abs(log2FoldChange)>2) %>%
  dplyr::rename(ASV=row)
View(sigASVs_T8_T6)
#To get ASV
sigASVs_T8_T6_vec <- sigASVs_T8_T6 %>%
  pull(ASV)

# Prune phyloseq file
T8_T6_DESeq <- prune_taxa(sigASVs_T8_T6_vec,calf_phyloseq_no_diet_and_T9)
sigASVs_T8_T6_merge <- tax_table(T8_T6_DESeq) %>% as.data.frame() %>%
  rownames_to_column(var="ASV") %>%
  right_join(sigASVs_T8_T6) %>%
  arrange(log2FoldChange) %>%
  mutate(Genus = make.unique(Genus)) %>%
  mutate(Genus = factor(Genus, levels=unique(Genus))) %>%
  mutate(Score = -log10(padj) * abs(log2FoldChange))  %>%
  arrange(desc(Score)) %>%
  slice_head(n = 10)
View(sigASVs_T8_T6_merge)

#graph volcano plot
vol_plot_T8_T6_upd <- res_calf_T8_T6 %>%
  mutate(significant = padj<0.01 & abs(log2FoldChange)>2) %>%
  ggplot() +
  geom_point(aes(x=log2FoldChange, y=-log10(padj), col=significant),size = 2, alpha = 0.8) + 
  scale_color_manual(
  ...
  geom_label_repel(
    data = sigASVs_T8_T6_merge, # Use the filtered data for only the points you want to label
    aes(x = log2FoldChange, y = -log10(padj), label = Genus),
    size = 3.5,
    box.padding = 0.6,
    point.padding = 0.5
...
ggsave("calf_DEG/labelled_graph/vol_plot_T8_T6_upd.png", vol_plot_T8_T6_upd, width = 8, height = 6.5, dpi = 300, bg = "white")
